...
import { arvish as createArvish } from './_utils';
...
test('cache key', async t => {
  ...
  t.truthy(
    arvish.cache.store[
      'https://foo.bar/cache-key{"json":true,"query":{"unicorn":"rainbow"},"maxAge":5000}'
    ]
  );
});
...
test('invalid version', async t => {
  ...
  t.deepEqual(
    arvish.cache.store['https://foo.bar/cache-version{"json":true,"maxAge":5000}']
      .data,
    { foo: 'bar' }
  );
